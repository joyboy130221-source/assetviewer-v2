...
async function ensureSchema() {
  ...
  initialized = (async () => {
    ...
    await db.query(`INSERT INTO app_roles(name,description,permissions) VALUES($1,$2,$3::jsonb) ON CONFLICT(name) DO NOTHING`, ['administrator','Default administrator role', JSON.stringify({ roles: true, users: true, maximoEnvironments: true, externalViews: true, apiLogs: true })]);
    await db.query(`UPDATE app_roles SET permissions = permissions || '{"externalViews": true}'::jsonb, updated_at=NOW() WHERE name='administrator' AND NOT (permissions ? 'externalViews')`);
