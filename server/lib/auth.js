...
function secret() {
  if (!process.env.SESSION_SECRET)
    throw Object.assign(new Error("SESSION_SECRET is not configured."), {
      status: 500,
    });
...
function createToken(user) {
  const payload = b64(
    JSON.stringify({ uid: user.id, exp: Date.now() + 8 * 60 * 60 * 1000 }),
  );
...
function setSession(res, user) {
  res.setHeader(
    "Set-Cookie",
    `${COOKIE}=${encodeURIComponent(createToken(user))}; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=28800`,
  );
}
