...
function getMaximoErrorDetails(value, fallback) {
  ...
  const message =
    ...
    fallback;
  const reasonCode =
    root.reasonCode || root["spi:reasonCode"] || payload.reasonCode || null;
  const statusCode =
    root.statusCode || root["oslc:statusCode"] || payload.statusCode || null;
...
async function getEnvironment(envName) {
  ...
  if (!name)
    throw Object.assign(new Error("env is required. Example: ?env=demo-coh"), {
      status: 400,
    });
  ...
  if (!result.rows[0])
    throw Object.assign(
      new Error(`Maximo environment '${name}' was not found or is inactive.`),
      {
        status: 404,
      },
    );
  ...
  try {
    ...
    await recordApiLog({
      ...
      errorMessage: upstream.ok
        ? null
        : readMaximoError(data, `Maximo API returned ${upstream.status}`),
    });
    if (!upstream.ok) {
      const details = getMaximoErrorDetails(
        data,
        `Maximo API returned ${upstream.status}`,
      );
...
function sendError(response, error, fallback = "Maximo request failed") {
  ...
  return response.status(status).json({
    ...
    statusCode:
      (error && error.maximoStatusCode) || details.statusCode || String(status),
