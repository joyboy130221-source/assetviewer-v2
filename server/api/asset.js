...
function extractFirstMember(payload) {
  ...
  if (payload.data && typeof payload.data === "object")
    return extractFirstMember(payload.data);
  return null;
}
...
function cleanAttributes(input) {
  ...
  for (const [key, value] of Object.entries(input)) {
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key) || PROTECTED_FIELDS.has(key))
      continue;
    if (
      value === null ||
...
module.exports = async function handler(request, response) {
  const envName =
    request.method === "GET" ? request.query.env : parseBody(request.body).env;
  const assetId =
    request.method === "GET"
      ? typeof request.query.assetId === "string"
        ? request.query.assetId.trim()
        : ""
  ...
  try {
    ...
    if (!Object.keys(attributes).length)
      return response
        .status(400)
        .json({ error: "No editable attributes were supplied" });
    ...
    const asset = await findSingle(
      env,
      OBJECT_STRUCTURE,
      where,
      "assetnum,siteid,href",
    );
    if (!asset)
      return response
        .status(404)
    ...
    if (!asset.href)
      return response
        .status(502)
        .json({ error: "Maximo did not return an asset resource URL (href)" });
    ...
    return sendError(response, error, "Unable to process asset request");
  }
};
