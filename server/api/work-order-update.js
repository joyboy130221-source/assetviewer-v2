...
async function updateStatus(env, wonum, siteid, status, memo) {
  if (!ALLOWED.has(status)) {
    throw Object.assign(new Error("Invalid work order status."), {
      status: 400,
    });
  }
  ...
  if (!wo?.href) {
    throw Object.assign(new Error(`Work order ${wonum} was not found.`), {
      status: 404,
    });
  }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (status)
      result.workOrder = await updateStatus(env, wonum, siteid, status, memo);
    ...
    for (const item of logs) {
      ...
      if (
        ...
        item.worklogid !== null &&
        String(item.worklogid).trim() !== ""
      ) {
        result.worklogs.push(
          await updateWorklog(env, wonum, siteid, item.worklogid, item),
        );
      } else {
    ...
    return res.json({
      message: "Work order changes submitted successfully.",
      data: result,
    });
  } catch (e) {
