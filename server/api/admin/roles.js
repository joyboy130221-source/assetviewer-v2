...
function perms(v) {
  const p = {};
  KNOWN.forEach((k) => (p[k] = Boolean(v?.[k])));
  return p;
}
module.exports = async (req, res) => {
  const user = await requireAuth(req, res, "roles");
  if (!user) return;
  try {
    if (req.method === "GET") {
      const r = await query(
      ...
      return res.json({
        ...
        availablePermissions: [
          { key: "roles", label: "Role Page" },
          { key: "users", label: "User Page" },
          ...
          {
            key: "authenticationProfiles",
            label: "Authentication Profiles Page",
          },
        ],
      });
    }
    ...
    if (req.method === "POST") {
      ...
      await query(
        `INSERT INTO app_roles(name,description,active,permissions) VALUES($1,$2,$3,$4::jsonb)`,
        [
          b.name.trim(),
          b.description || "",
          b.active !== false,
          JSON.stringify(perms(b.permissions)),
        ],
      );
    ...
    if (req.method === "DELETE") {
      if (Number(b.id) === Number(user.role_id))
    ...
    res.status(e.code === "23503" ? 409 : e.code === "23505" ? 409 : 500).json({
      error:
        e.code === "23503"
