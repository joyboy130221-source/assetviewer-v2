...
module.exports = async (req, res) => {
  const user = await requireAuth(req, res, "apiLogs");
  if (!user) return;
  if (req.method !== "GET")
  ...
  try {
    ...
    if (q.status) {
      ...
      if (Number.isFinite(n)) add("response_status=?", n);
    }
    ...
    if (q.search) {
      ...
      where.push(
        `(request_url ILIKE $${values.length} OR COALESCE(error_message,'') ILIKE $${values.length})`,
      );
    }
    ...
    const [logs, envs] = await Promise.all([
      query(sql, values),
      query(
        `SELECT DISTINCT environment_name FROM api_request_logs WHERE environment_name IS NOT NULL ORDER BY environment_name`,
      ),
    ]);
    return res.json({
      data: logs.rows,
      environments: envs.rows.map((x) => x.environment_name),
    });
  } catch (e) {
