...
const ALLOWED = new Set(["APPTNOTE", "CLIENTNOTE", "UPDATE", "WORK"]);
...
function normalize(input) {
  ...
  const logtype = String(input.logtype || "")
    .trim()
    .toUpperCase();

  if (!description) {
    throw Object.assign(new Error("Worklog description is required."), {
      status: 400,
    });
  }

  if (!longDescription) {
    throw Object.assign(new Error("Worklog long description is required."), {
      status: 400,
    });
  }

  if (!ALLOWED.has(logtype)) {
    throw Object.assign(new Error("Invalid worklog log type."), {
      status: 400,
    });
  }
...
async function listWorklogs(env, wonum, siteid) {
  if (!wonum) {
    throw Object.assign(new Error("Work Order number is required."), {
      status: 400,
    });
  }

  const url = new URL(objectStructureUrl(env, "mxapiworklog"));
 ...
 */
async function createWorklog(env, wonum, siteid, input) {
  if (!wonum) {
    throw Object.assign(new Error("Work Order number is required."), {
      status: 400,
    });
 ...
 */
async function getWorkOrderWithWorklogs(env, wonum, siteid) {
  if (!wonum) {
    throw Object.assign(
      new Error("Work Order number is required to update a Worklog."),
      {
        status: 400,
      },
    );
  ...
  const url = new URL(objectStructureUrl(env, "mxapiwo"));
  ...
  if (members.length === 0) {
    throw Object.assign(
      new Error(`Work Order ${wonum} was not found for site ${siteid}.`),
      {
        status: 404,
      },
    );
 ...
 * ============================================================
 */
function findWorklog(workOrder, worklogid) {
  const id = String(worklogid == null ? "" : worklogid).trim();

  if (!id) {
    throw Object.assign(new Error("A valid worklogid is required."), {
      status: 400,
    });
  }

  const worklogs = Array.isArray(workOrder.worklog) ? workOrder.worklog : [];
...
function getWorklogUpdateUrl(env, workOrder, worklog) {
  ...
  throw Object.assign(
    new Error(
      `No usable update URL was returned by Maximo for Worklog ${worklog.worklogid || ""}.`,
    ),
    { status: 502 },
  );
}
...
async function updateWorklog(env, wonum, siteid, worklogid, input) {
  if (
    ...
    throw Object.assign(
      new Error(`Worklog ID is missing. Received value: ${worklogid}`),
      { status: 400 },
    );
  }
...
module.exports = async (req, res) => {
  ...
  const wonum = String(req.query.wonum || body.wonum || "").trim();

  const siteid = String(req.query.siteid || body.siteid || "BEDFORD").trim();

  try {
    const env = await getEnvironment(req.query.env || body.env);
     ...
     */
    if (req.method === "GET") {
      const data = await listWorklogs(env, wonum, siteid);
    ...
    if (operation === "update") {
      ...
      data = await updateWorklog(env, wonum, siteid, body.worklogid, body);
    }
     ...
     */
      ...
      data = await createWorklog(env, wonum, siteid, body);
    }
    ...
    sendError(res, error, "Unable to process worklog.");
  }
};
...
module.exports.getWorkOrderWithWorklogs = getWorkOrderWithWorklogs;

module.exports.findWorklog = findWorklog;

module.exports.getWorklogUpdateUrl = getWorklogUpdateUrl;
