...
export default function ApiLogsPage() {
  ...
  const load = async (form?: HTMLFormElement) => {
    ...
    try {
      ...
      const b = await api<any>(`/api/admin/api-logs?${p}`);
      setLogs(b.data);
  ...
  return (
    <AdminLayout
      ...
      subtitle="Trace Maximo requests and responses. Credentials are automatically masked; only a short fingerprint is retained for troubleshooting."
    >
      <section className="data-card">
        <div className="filter-panel">
          <div className="filter-panel-head">
            <div>
              ...
              <div>
                ...
                <p>
                  Narrow the request history by environment, response, method or
                  keyword.
                </p>
              </div>
            </div>
            ...
            >
              <RefreshCw size={17} className={filtering ? "spin-icon" : ""} />
            </button>
          </div>
          <form ref={formRef} className="log-filters" onSubmit={submit}>
            ...
            <label>
              Status
              <input name="status" placeholder="e.g. 400" />
            </label>
            <label className="log-search">
              ...
              <div className="filter-search">
                ...
                <input
                  name="search"
                  placeholder="URL, mxasset, worklog, error..."
                />
              </div>
            </label>
        ...
        <div className="table-wrap admin-table-scroll">
          <table className="admin-table log-table">
            ...
            <tbody>
              {logs.map((x) => {
                let path = x.request_url;
                try {
                  const u = new URL(x.request_url);
                  path = u.pathname + (u.search || "");
                } catch {}
                return (
                  <tr key={x.id}>
                    <td>{new Date(x.created_at).toLocaleString()}</td>
                    <td>{x.environment_name || "—"}</td>
                    <td>
                      <span className="method-badge">{x.request_method}</span>
                    </td>
                    <td className="request-cell">
                      <span className="request-truncate" title={path}>
                        {path}
                      </span>
                    </td>
                    <td>
                      <span
                        className={
                          x.success ? "badge-active" : "badge-inactive"
                        }
                      >
                        {x.response_status ?? "NETWORK"}
                      </span>
                    </td>
                    <td>
                      {x.duration_ms == null ? "—" : `${x.duration_ms} ms`}
                    </td>
                    <td>
                      <span
                        className={
                          x.success ? "result-success" : "result-error"
                        }
                      >
        ...
        {!logs.length && (
          <div className="empty-state">
            No API logs match the current filter.
          </div>
        )}
      </section>
      {detail && (
        ...
        >
          <div className="modal-card log-detail-card">
            <div className="log-detail-head">
              <div>
                <p className="eyebrow">API TRACE</p>
                <h2>
                  {detail.request_method} • Log #{detail.id}
                </h2>
              </div>
              <button
                className="icon-button"
            ...
            <div className="log-meta">
              {[
                ["Environment", detail.environment_name],
                ["Status", detail.response_status ?? "No response"],
                [
                  "Duration",
                  detail.duration_ms == null ? "—" : `${detail.duration_ms} ms`,
                ],
                ["Created", new Date(detail.created_at).toLocaleString()],
                ["Result", detail.success ? "Success" : "Error"],
              ].map(([a, b]) => (
                <div key={a}>
                  <span>{a}</span>
                  <strong>{b}</strong>
            ...
            {[
              ["Request URL", detail.request_url],
              ["Request Headers", detail.request_headers],
              ["Request Parameters", detail.request_params],
              ...
              >
                <div className="log-section-title">
                  ...
                  <button
                    ...
                    onClick={() =>
                      navigator.clipboard
                        .writeText(typeof v === "string" ? v : pretty(v))
                        .then(() => ui.toast(`${t} copied.`))
                    }
                  >
                    Copy
                  </button>
                </div>
                <pre>{typeof v === "string" ? v : pretty(v)}</pre>
              </section>
            ))}
          </div>
        </div>
