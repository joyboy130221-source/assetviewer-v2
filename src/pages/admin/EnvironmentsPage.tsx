...
export default function EnvironmentsPage() {
  const [data, setData] = useState<any[]>([]),
    ...
    ui = useAppUI();
  const load = () =>
    api<any>("/api/admin/environments").then((b) => setData(b.data));
  useEffect(() => {
  ...
  const save = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (
      !(await ui.confirm({
        title: editing ? "Update environment?" : "Create environment?",
        message: `Save configuration for “${b.env_name}”?`,
  ...
  const del = async (x: any) => {
    if (
      !(await ui.confirm({
        ...
        message: `Delete Maximo environment “${x.env_name}”?`,
        confirmText: "Delete",
        danger: true,
      }))
    )
  ...
  return (
    <>
      <AdminLayout
        ...
        eyebrow="MAXIMO CONFIGURATION"
        title="Maximo Environments"
        subtitle={
          <>
            Select the configuration from public pages using the{" "}
            <strong>env</strong> URL parameter.
          </>
        }
      >
        <section className="data-card">
          <div className="card-toolbar">
            <div>
              <h2>API Endpoints</h2>
              <p>Manage Maximo connection endpoints used by the application.</p>
            </div>
            <button
            ...
            <table className="admin-table">
              ...
              <tbody>
                {data.map((x) => (
                  <tr key={x.id}>
                    <td>
                      <strong>{x.env_name}</strong>
                    </td>
                    <td>{x.description || "—"}</td>
                    <td className="endpoint">{x.endpoint}</td>
                    <td>{x.api_key_masked}</td>
                    <td>
                      <span
