...
export default function LoginPage() {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await api("/api/auth/login", jsonInit("POST", Object.fromEntries(fd)));
      ui.toast("Signed in successfully.");
      nav(sp.get("next") || "/admin");
    } catch (e: any) {
      setMsg(e.message);
      ui.toast(e.message || "Unable to sign in.", "error");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="login-page">
        <div className="login-shell">
          <section className="login-brand-panel">
            ...
            <div>
              <p className="login-brand-name">Integration Hub</p>
              <p className="login-brand-tag">Enterprise Administration</p>
            </div>
          ...
          <section className="login-card">
            ...
            <p className="login-footnote">
              Authorized personnel only. Access may be monitored for security
              and audit purposes.
            </p>
          </section>
        </div>
