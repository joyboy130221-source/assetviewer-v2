...
import { Activity, ChevronDown, ExternalLink, Gauge, LogOut, ServerCog, ShieldCheck, Users } from 'lucide-react';
...
const setup: MenuItem[] = [
  ...
  { to: '/admin/external-views', label: 'External View', permission: 'externalViews', icon: <ExternalLink size={17} /> },
...
const monitoring: MenuItem[] = [
  { to: '/admin/api-logs', label: 'API Request Log', permission: 'apiLogs', icon: <Activity size={17} /> },
];
...
export function AdminLayout({ permission, eyebrow, title, subtitle, children }: { permission?: string; eyebrow: string; title: string; subtitle?: ReactNode; children: ReactNode }) {
  ...
  useEffect(() => {
    api<{ user: AdminUser }>('/api/auth/me', { cache: 'no-store' })
      ...
      .catch(() => navigate(`/login?next=${encodeURIComponent(location.pathname + location.search)}`));
  }, [permission]);
  ...
  const allowed = (item: MenuItem) => !item.permission || user.permissions?.[item.permission];
  const logout = async () => {
    if (await ui.confirm({ title: 'Sign out?', message: 'Your administration session will be closed.', confirmText: 'Sign Out' })) {
      await fetch('/api/auth/logout', { method: 'POST' });
      navigate('/login');
    }
  };
  const group = (label: string, items: MenuItem[]) => <div className="nav-group"><div className="nav-group-title"><span>{label}</span><ChevronDown size={14} /></div>{items.filter(allowed).map((item) => <NavLink key={item.to} to={item.to} end={item.end}><span className="nav-icon">{item.icon}</span><span>{item.label}</span></NavLink>)}</div>;
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main"><div className="admin-topbar"><div><p className="eyebrow">{eyebrow}</p><h1>{title}</h1>{subtitle && <p className="subtitle">{subtitle}</p>}</div><span className="admin-user-chip">{user.name} <small>{user.role}</small></span></div>{children}</main>
  </div>;
}
