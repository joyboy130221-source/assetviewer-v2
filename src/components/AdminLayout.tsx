...
export function AdminLayout({permission,eyebrow,title,subtitle,children}:{permission?:string;eyebrow:string;title:string;subtitle?:ReactNode;children:ReactNode}){
  ...
  useEffect(()=>{api<{user:AdminUser}>('/api/auth/me',{cache:'no-store'}).then(b=>{if(permission&&!b.user.permissions?.[permission])setDenied(true);else setUser(b.user)}).catch(()=>nav(`/login?next=${encodeURIComponent(loc.pathname+loc.search)}`));},[permission]);
  ...
  const logout=async()=>{if(await ui.confirm({title:'Sign out?',message:'Your administration session will be closed.',confirmText:'Sign Out'})){await fetch('/api/auth/logout',{method:'POST'});nav('/login')}};
